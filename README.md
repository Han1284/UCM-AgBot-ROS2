...
多视角感知、候选投影、MTC 规划和一次性失败重试均由上述管线入口统一管理。
完整的启动名称与可复制命令集中在 [`LAUNCH_COMMANDS.md`](LAUNCH_COMMANDS.md)，
本节只保留必要的接口和故障核查命令，避免把已经变化的内部算法细节当成固定
操作步骤。
